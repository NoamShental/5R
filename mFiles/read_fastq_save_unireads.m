function read_fastq_save_unireads(dname, sample_name, PrepConfig, dest_pe_flag, readsStatsObj)
...
for pp = 1:size(pairs_ind,1)
    fname_r1 = [dname '/' tmp_names(pairs_ind(pp,1)).name];
    if pe_flag == 1
        fname_r2 = [dname '/' tmp_names(pairs_ind(pp,2)).name];
    end
...
savename_1 = [dname '/resDir/' sample_name '_R1_unireads.fasta'];
if exist(savename_1,'file')
    delete(savename_1)
end
if dest_pe_flag == 1
    savename_2 = [dname '/resDir/' sample_name '_R2_unireads.fasta'];
...
save([dname '/resDir/' sample_name '_unireads.mat'],'freq','Suni','Huni')

% Delete the fastq files
if delete_fastq_files
    delete([dname '/' sample_name '*.fastq'])
end
